...
#if DBG_ENABLED
#  define dbg(x) do { std::cerr << "[gravity] " << x << "\n"; } while(0)
#else
...
//==================================================================================================
// Gravity-compensation control law (joint-space PD + τ_g)
//==================================================================================================
namespace
{
...
controller_interface::CallbackReturn GravityCompensationController::on_init()
{
  ...
  RCLCPP_INFO(
    get_node()->get_logger(),
    "Gravity compensation controller initialized");
  ...
  const rclcpp_lifecycle::State &)
{
  ...
  RCLCPP_INFO(
    get_node()->get_logger(),
    "Configured (gravity compensation): backend=%s urdf=%s reference_topic=%s joints=%zu",
  ...
  const rclcpp_lifecycle::State &)
{
  ...
  if (!imu_topic_.empty()) {
    ...
    RCLCPP_INFO(
      get_node()->get_logger(),
      "Subscribed to IMU topic: %s", imu_topic_.c_str());
  }
  ...
  RCLCPP_INFO(get_node()->get_logger(), "Gravity compensation controller activated");
  return controller_interface::CallbackReturn::SUCCESS;
}
  ...
  const rclcpp_lifecycle::State &)
{
  ...
  traj_sub_.reset();
  imu_sub_.reset();
  RCLCPP_INFO(get_node()->get_logger(), "Gravity compensation controller deactivated");
  return controller_interface::CallbackReturn::SUCCESS;
}
  ...
  const rclcpp::Duration &)
{
  ...
  if (imu_sub_) {
    ...
    if (q) {
      ...
      std::string g_set_err;
      dynamics_->setGravity(g_arm, &g_set_err);
    }
  }
