...
#if DBG_ENABLED
#  define dbg(x) do { std::cerr << "[admittance] " << x << "\n"; } while(0)
#else
...
//==================================================================================================
// Joint-space admittance control law
//
// Virtual compliance model (per joint, decoupled scalar Md, Bd, Kd):
//   Md * q̈_v + Bd * q̇_v + Kd * q_v = τ_ext
//   → q̈_v = (τ_ext - Bd*q̇_v - Kd*q_v) / Md
//
// Modified setpoint:
//   q_cmd  = q_des  + q_v,   q̇_cmd = q̇_des + q̇_v
//   q̈_cmd  = q̈_des + q̈_v   (q̈_v integrated from virtual model)
//
// Inner FL control law:
//   τ = M(q)*(q̈_cmd + Kp*(q_cmd-q) + Kd_inner*(q̇_cmd-q̇)) + C(q,q̇)*q̇ + g(q)
//
// The virtual state (q_v, dq_v) is passed by reference and integrated in place.
//==================================================================================================
namespace
{
...
bool controlLaw(
  ...
  Eigen::VectorXd & q_v,     // in/out: virtual displacement
  Eigen::VectorXd & dq_v,    // in/out: virtual velocity
  Eigen::VectorXd & tau,
  ...
  std::string * error)
{
  ...
  Eigen::Map<const Eigen::VectorXd> q_v_map(q.data(), n);
  Eigen::Map<const Eigen::VectorXd> dq_v_map(dq.data(), n);
  ...
  Eigen::Map<const Eigen::VectorXd> tau_ext_v(tau_ext.data(), n);

  // Virtual compliance integration (forward Euler)
  const Eigen::VectorXd ddq_v =
    (tau_ext_v - bd * dq_v - kd_adm * q_v).array() / md;
  dq_v += dt * ddq_v;
  q_v  += dt * dq_v;

  // Modified setpoint
  const Eigen::VectorXd q_cmd  = q_des_v  + q_v;
  const Eigen::VectorXd dq_cmd = dq_des_v + dq_v;
  const Eigen::VectorXd ddq_cmd = ddq_des_v + ddq_v;

  // Inner FL servo
  const Eigen::VectorXd a =
    ddq_cmd + kp * (q_cmd - q_v_map) + kd_inner * (dq_cmd - dq_v_map);
  tau = M * a + C * dq_v_map + g_v;
  return true;
}
...
controller_interface::CallbackReturn AdmittanceController::on_init()
{
  ...
  auto_declare<double>("md", 1.0);
  auto_declare<double>("bd", 10.0);
  auto_declare<double>("kd_adm", 0.0);
  auto_declare<std::string>("ext_torque_topic", std::string(""));
  ...
  RCLCPP_INFO(get_node()->get_logger(), "Admittance controller initialized");
  ...
  const rclcpp_lifecycle::State &)
{
  ...
  md_         = get_node()->get_parameter("md").as_double();
  bd_         = get_node()->get_parameter("bd").as_double();
  kd_adm_     = get_node()->get_parameter("kd_adm").as_double();
  ext_torque_topic_ = get_node()->get_parameter("ext_torque_topic").as_string();
  ...
  // Seed zero external torque in the buffer
  ...
  RCLCPP_INFO(
    get_node()->get_logger(),
    "Configured (Admittance): backend=%s kp=%.1f kd=%.1f Md=%.2f Bd=%.2f Kd_adm=%.2f",
    dynamics_backend_.c_str(), kp_, kd_, md_, bd_, kd_adm_);
  return controller_interface::CallbackReturn::SUCCESS;
}
  ...
  const rclcpp_lifecycle::State &)
{
  ...
  // Reset virtual compliance state
  const size_t n = joint_names_.size();
  q_v_  = Eigen::VectorXd::Zero(n);
  dq_v_ = Eigen::VectorXd::Zero(n);
  ...
  traj_sub_ = get_node()->create_subscription<trajectory_msgs::msg::JointTrajectory>(
    ...
    std::bind(
      &AdmittanceController::referenceTrajectoryCallback, this, std::placeholders::_1));

  if (!ext_torque_topic_.empty()) {
    ext_torque_sub_ = get_node()->create_subscription<std_msgs::msg::Float64MultiArray>(
      ext_torque_topic_, rclcpp::SensorDataQoS(),
      [this](const std_msgs::msg::Float64MultiArray::SharedPtr msg) {
        if (msg->data.size() == joint_names_param_.size()) {
          ext_torque_buf_.writeFromNonRT(msg->data);
        }
      });
    RCLCPP_INFO(
      get_node()->get_logger(),
      "Subscribed to external torque topic: %s", ext_torque_topic_.c_str());
  }
...
bool AdmittanceController::init_interfaces()
{
...
void AdmittanceController::referenceTrajectoryCallback(
  ...
  const rclcpp::Duration & period)
{
  ...
  const std::vector<double> * tau_ext_ptr = ext_torque_buf_.readFromRT();
  std::vector<double> tau_ext =
    (tau_ext_ptr && tau_ext_ptr->size() == joint_names_.size())
    ? *tau_ext_ptr
  ...
  if (!controlLaw(
      *dynamics_, joint_names_, kp_, kd_, md_, bd_, kd_adm_, dt,
      sp_ptr->positions, sp_ptr->velocities, sp_ptr->accelerations,
      tau_ext, q_meas, dq_meas,
      q_v_, dq_v_, tau_vec, tau_g, &ctrl_err))
  {
    RCLCPP_ERROR_THROTTLE(
      get_node()->get_logger(), *get_node()->get_clock(), 1000,
      "Admittance control law failed: %s", ctrl_err.c_str());
  ...
  if (telemetry_pub_) {
    ...
    for (size_t i = 0; i < n; ++i) {
      ...
      msg.position_reference[i] = sp_ptr->positions[i] + q_v_(ii);   // modified setpoint
      msg.velocity_reference[i] = sp_ptr->velocities[i] + dq_v_(ii);
