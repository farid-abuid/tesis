...
//   s = ė + λ*e                                                   (sliding surface, per joint)
//   v = q̈_des - kp*s - k_switch*tanh(s/φ) - s³                  (virtual control)
...
controller_interface::CallbackReturn SmcController::on_init()
{
  ...
  RCLCPP_INFO(get_node()->get_logger(), "SMC controller initialized");
  return controller_interface::CallbackReturn::SUCCESS;
}

controller_interface::CallbackReturn SmcController::on_configure(
  const rclcpp_lifecycle::State &)
{
  ...
  RCLCPP_INFO(
    get_node()->get_logger(),
    "Configured (SMC): backend=%s urdf=%s lambda=%.2f kp=%.2f k=%.2f phi=%.3f joints=%zu",
    dynamics_backend_.c_str(), urdf_path_.c_str(), lambda_, k_p_, k_switch_, phi_,
    joint_names_param_.size());
  return controller_interface::CallbackReturn::SUCCESS;
}

controller_interface::CallbackReturn SmcController::on_activate(
  const rclcpp_lifecycle::State &)
{
  if (!init_interfaces()) {
  ...
  traj_sub_ = get_node()->create_subscription<trajectory_msgs::msg::JointTrajectory>(
    ...
    std::bind(&SmcController::referenceTrajectoryCallback, this, std::placeholders::_1));
  ...
  RCLCPP_INFO(get_node()->get_logger(), "SMC controller activated");
  return controller_interface::CallbackReturn::SUCCESS;
}

controller_interface::CallbackReturn SmcController::on_deactivate(
  const rclcpp_lifecycle::State &)
{
  if (logging_session_pub_) {
    std_msgs::msg::Bool msg;
  ...
  traj_sub_.reset();
  imu_sub_.reset();
  RCLCPP_INFO(get_node()->get_logger(), "SMC controller deactivated");
  return controller_interface::CallbackReturn::SUCCESS;
}

controller_interface::InterfaceConfiguration
SmcController::command_interface_configuration() const
{
...
controller_interface::InterfaceConfiguration
SmcController::state_interface_configuration() const
{
  controller_interface::InterfaceConfiguration config;
  config.type = controller_interface::interface_configuration_type::INDIVIDUAL;
  for (const auto & joint : joint_names_param_) {
...
bool SmcController::init_interfaces()
{
...
void SmcController::referenceTrajectoryCallback(
...
controller_interface::return_type SmcController::update(
...
PLUGINLIB_EXPORT_CLASS(smc_controller::SmcController, controller_interface::ControllerInterface)
