...
//==================================================================================================
// Joint-space impedance control law
//
// Desired impedance (scalar Md, K, D applied per joint):
//   Md * ë = -K*(q - q_des) - D*(q̇ - q̇_des) + τ_ext
//
// Full model-based implementation (FL feedforward + impedance servo):
//   τ = M(q)*(q̈_des + Md⁻¹*(-K*e - D*ė + τ_ext)) + C(q,q̇)*q̇ + g(q)
//
// When τ_ext = 0 and Md equals the actual inertia, this reduces to a pure
// computed-torque PD controller.  Varying Md shapes the apparent joint inertia.
//==================================================================================================
namespace
{
  ...
  std::string * error)
{
  ...
  // Impedance acceleration: Md⁻¹*(-K*e - D*ė + τ_ext)
  const Eigen::VectorXd a_imp =
    (-kp_imp * e - kd_imp * de + tau_ext_v) / m_desired;

  // τ = M*(q̈_des + a_imp) + C*q̇ + g
  tau = M * (ddq_des_v + a_imp) + C * dq_v + g_v;
  return true;
}
...
controller_interface::CallbackReturn ImpedanceController::on_init()
{
  ...
  auto_declare<double>("m_desired", 1.0);
  auto_declare<double>("kp_imp", 100.0);
  auto_declare<double>("kd_imp", 20.0);
  ...
  RCLCPP_INFO(get_node()->get_logger(), "Impedance controller initialized");
  ...
  const rclcpp_lifecycle::State &)
{
  ...
  m_desired_  = get_node()->get_parameter("m_desired").as_double();
  kp_imp_     = get_node()->get_parameter("kp_imp").as_double();
  kd_imp_     = get_node()->get_parameter("kd_imp").as_double();
  ...
  RCLCPP_INFO(
    get_node()->get_logger(),
    "Configured (Impedance): backend=%s Md=%.2f Kp=%.1f Kd=%.1f joints=%zu",
    dynamics_backend_.c_str(), m_desired_, kp_imp_, kd_imp_, joint_names_param_.size());
  return controller_interface::CallbackReturn::SUCCESS;
}
  ...
  const rclcpp_lifecycle::State &)
{
  ...
  traj_sub_ = get_node()->create_subscription<trajectory_msgs::msg::JointTrajectory>(
    ...
    std::bind(
      &ImpedanceController::referenceTrajectoryCallback, this, std::placeholders::_1));
  ...
  RCLCPP_INFO(get_node()->get_logger(), "Impedance controller activated");
  return controller_interface::CallbackReturn::SUCCESS;
}
...
bool ImpedanceController::init_interfaces()
{
...
void ImpedanceController::referenceTrajectoryCallback(
  ...
  const rclcpp::Duration &)
{
  ...
  if (!controlLaw(
      *dynamics_, joint_names_, m_desired_, kp_imp_, kd_imp_,
      sp_ptr->positions, sp_ptr->velocities, sp_ptr->accelerations,
      tau_ext, q_meas, dq_meas, tau_vec, tau_g, &ctrl_err))
  {
    RCLCPP_ERROR_THROTTLE(
      get_node()->get_logger(), *get_node()->get_clock(), 1000,
      "Impedance control law failed: %s", ctrl_err.c_str());
    return controller_interface::return_type::ERROR;
  }
