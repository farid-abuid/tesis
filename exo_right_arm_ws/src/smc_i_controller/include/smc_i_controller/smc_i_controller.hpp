...
///   ei = ∫e dt  (accumulated state)
///   s  = e + λ*ei,  v = -kp*s - k*tanh(s/φ) - s³
///   τ  = M(q)*(v - λ*ė + q̈_des) + C(q,q̇)*q̇ + g(q)
class SmcIController : public controller_interface::ControllerInterface
{
public:
  ~SmcIController() override = default;
...
private:
  ...
  double lambda_{10.0};   // sliding surface slope
  double k_p_{5.0};       // proportional gain on s
  double k_switch_{5.0};  // switching gain
  double phi_{0.05};      // boundary layer thickness
  double i_clamp_{0.0};   // per-joint anti-windup clamp on |∫e|; <=0 disables

  Eigen::VectorXd integral_error_;  // accumulated tracking error state
