#include "smc_i_controller/smc_i_controller.hpp"
...
#if DBG_ENABLED
#  define dbg(x) do { std::cerr << "[smc_i] " << x << "\n"; } while(0)
#else
#  define dbg(x) do {} while(0)
#endif

namespace smc_i_controller
{

//==================================================================================================
// Sliding mode control with integral action
//   ei = ∫e dt                                                    (accumulated state, per joint)
//   s  = e + λ*ei                                                 (sliding surface, per joint)
//   v  = -kp*s - k_switch*tanh(s/φ) - s³                          (virtual control)
//   τ  = M(q)*(v - λ*ė + q̈_des) + C(q,q̇)*q̇ + g(q)
//==================================================================================================
namespace
{
...
bool controlLaw(
  ...
  double k_switch,
  double phi,
  const Eigen::VectorXd & integral_error,
  ...
  std::string * error)
{
  ...
  const Eigen::VectorXd s  = e + lambda * integral_error;

  //dbg("e[3] = " << e(3) << "  de[3] = " << de(3));
  //dbg("tau_g[3] = " << tau_g[3]);
  dbg("s = " << s.transpose());

  // Virtual control: proportional + switching + quintic term
  Eigen::VectorXd v(n);
...
controller_interface::CallbackReturn SmcIController::on_init()
{
  ...
  auto_declare<double>("lambda", 10.0);
  auto_declare<double>("k_p", 5.0);
  auto_declare<double>("k_switch", 5.0);
  auto_declare<double>("phi", 0.05);
  auto_declare<double>("i_clamp", 0.0);
  ...
  RCLCPP_INFO(get_node()->get_logger(), "SMC-I controller initialized");
  return controller_interface::CallbackReturn::SUCCESS;
}

controller_interface::CallbackReturn SmcIController::on_configure(
  const rclcpp_lifecycle::State &)
{
  ...
  k_switch_ = get_node()->get_parameter("k_switch").as_double();
  phi_      = get_node()->get_parameter("phi").as_double();
  i_clamp_  = get_node()->get_parameter("i_clamp").as_double();
  ...
  integral_error_ = Eigen::VectorXd::Zero(static_cast<Eigen::Index>(joint_names_param_.size()));

  RCLCPP_INFO(
    get_node()->get_logger(),
    "Configured (SMC-I): backend=%s urdf=%s lambda=%.2f kp=%.2f k=%.2f phi=%.3f "
    "i_clamp=%.3f joints=%zu",
    dynamics_backend_.c_str(), urdf_path_.c_str(), lambda_, k_p_, k_switch_, phi_,
    i_clamp_, joint_names_param_.size());
  return controller_interface::CallbackReturn::SUCCESS;
}

controller_interface::CallbackReturn SmcIController::on_activate(
  const rclcpp_lifecycle::State &)
{
  if (!init_interfaces()) {
  ...
  integral_error_.setZero(static_cast<Eigen::Index>(joint_names_.size()));
  ...
  traj_sub_ = get_node()->create_subscription<trajectory_msgs::msg::JointTrajectory>(
    ...
    std::bind(&SmcIController::referenceTrajectoryCallback, this, std::placeholders::_1));
  ...
  RCLCPP_INFO(get_node()->get_logger(), "SMC-I controller activated");
  return controller_interface::CallbackReturn::SUCCESS;
}

controller_interface::CallbackReturn SmcIController::on_deactivate(
  const rclcpp_lifecycle::State &)
{
  if (logging_session_pub_) {
    std_msgs::msg::Bool msg;
  ...
  traj_sub_.reset();
  imu_sub_.reset();
  RCLCPP_INFO(get_node()->get_logger(), "SMC-I controller deactivated");
  return controller_interface::CallbackReturn::SUCCESS;
}

controller_interface::InterfaceConfiguration
SmcIController::command_interface_configuration() const
{
...
controller_interface::InterfaceConfiguration
SmcIController::state_interface_configuration() const
{
  controller_interface::InterfaceConfiguration config;
  config.type = controller_interface::interface_configuration_type::INDIVIDUAL;
  for (const auto & joint : joint_names_param_) {
...
bool SmcIController::init_interfaces()
{
...
void SmcIController::referenceTrajectoryCallback(
...
controller_interface::return_type SmcIController::update(
  ...
  const rclcpp::Duration & period)
{
  ...
  // Accumulate the tracking error using the actual update period.
  const double dt = period.seconds();
  if (dt > 0.0) {
    for (size_t i = 0; i < joint_names_.size(); ++i) {
      const double e_i = q_meas[i] - sp_ptr->positions[i];
      integral_error_(static_cast<Eigen::Index>(i)) += e_i * dt;
      if (i_clamp_ > 0.0) {
        double & ei = integral_error_(static_cast<Eigen::Index>(i));
        ei = std::clamp(ei, -i_clamp_, i_clamp_);
      }
    }
  }
      ...
      q_meas, dq_meas, tau_vec, tau_g, s_vec, &ctrl_err))
  {
    RCLCPP_ERROR_THROTTLE(
      ...
      "SMC-I control law failed: %s", ctrl_err.c_str());
...
PLUGINLIB_EXPORT_CLASS(smc_i_controller::SmcIController, controller_interface::ControllerInterface)
